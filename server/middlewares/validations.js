...
export const userRequestsValidations = {
  userSignUp: [
    validate.body(
      'username',
      'username must be between 5 to 15 alphanumeric characters',
    )
      .isLength({ min: 5, max: 15 }).isAlphanumeric().trim(),
    validate.body('password', 'include a password between 6 to 25 characters')
      .isLength({ min: 5, max: 25 }).trim(),
  ...
  getUserRecipes: [
    validate.param(
      'userId',
      'user id is invalid or specified user does not exist',
    ).isUUID().custom(value => models.user.findById(value)),
  ],

  updateUserImageUrl: [
    validate.body('imageUrl', 'please specify an image url in request body')
      .isURL(),
  ],
